...
test('runOpenAIVerificationWithRetry accepts optional timestamp parameter', () => {
  assert(
    /async function runOpenAIVerificationWithRetry\(\s*prompt: string,\s*currentIsoTimestamp\?: string\s*\)/.test(
      routeTs
    ),
    'runOpenAIVerificationWithRetry should accept the current ISO timestamp.'
  );
});

test('runOpenAIVerificationWithRetry adds system message with current timestamp', () => {
  const messageBlockMatch = routeTs.match(
    /const messages = currentIsoTimestamp[\s\S]*?\n\s*const response = await openai/
  );
...
test('verifyOutput passes current timestamp into verification helper', () => {
  ...
  assert(
    routeTs.includes('runOpenAIVerificationWithRetry(prompt, nowIso)'),
